...
use crate::{cpu::{CPU, Memory}, bus::Bus, trace::trace};
...
fn read_screen_state(cpu: &CPU, frame: &mut [u8; 32 * 3 * 32]) -> bool {
    ...
    for i in 0x0200..0x600 {
        let color_i = cpu.mem_read(i as u16);
        let (r, g, b) = color(color_i).rgb();
        if frame[frame_i] != r || frame[frame_i + 1] != b || frame[frame_i + 2] != g {
            frame[frame_i] = r;
            frame[frame_i + 1] = b;
            frame[frame_i + 2] = g;
            update = true;
